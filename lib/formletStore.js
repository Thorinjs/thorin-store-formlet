...
module.exports = (thorin, opt) => {
  ...
  class ThorinFormletStore extends thorin.Interface.Store {
     ...
     * NOTE:
     *  if program is started with --purge=true, we will delete all fields in preparation for syncing.
     * */
    init(storeConfig) {
    ...
    /*
    * Performs a formlet-upload
    * */
    upload(namespace, payload) {
      let fObj = this.getInstance();
      if (!fObj) return Promise.reject(thorin.error('FORMLET.NOT_READY', 'Formlet is not ready yet'));
      if (this[config].debug) {
        let full = '';
        if (thorin.env === 'development') {
          let d = Object.assign({}, payload);
          delete d.content;
          full = ': ' + JSON.stringify(d);
        }
        logger.trace(`UPLOAD ${namespace} ${full}`);
      }
      if(typeof payload !== 'object' || !payload) payload = {};
      payload.namespace = namespace;
      return fObj.dispatch('namespace.file.upload', payload);
    }

    /*
    * Performs a formlet-download
    * */
    download(namespace, id) {
      let fObj = this.getInstance();
      if (!fObj) return Promise.reject(thorin.error('FORMLET.NOT_READY', 'Formlet is not ready yet'));
      const payload = {
        id,
      ...
      if (this[config].debug) {
        ...
        logger.trace(`DOWNLOAD ${namespace} ${full}`);
      }
      return fObj.dispatch('namespace.file.download', payload);
    }
