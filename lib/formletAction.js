...
module.exports = (thorin, opt) => {
  ...
  class ThorinAction extends Action {
    ...
    _runCustomType(intentObj, handler, done) {
      ...
      if (opt.rawFilter === true) {
        filter = intentObj.rawFilter;
      } else {
        filter = intentObj.filter();
        if (typeof opt.filter === 'object' && opt.filter) {
          filter = thorin.util.extend(filter, opt.filter);
        }
      }
      ...
      storeObj.query(actionName, payload, filter).then((res) => {
