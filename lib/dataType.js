...
module.exports = {
  ...
  DATE_ONLY: function DATE_ONLY(val) {
    return map('DATE_ONLY', val);
  },
  ...
  MULTI_ENUM: function ENUM(values) {
    let o = {
      type: 'MULTI',
      validations: {
        multi: true
  ...
  GPS_LAT: function GPS_LAT(val) {
    return map('GPS_LAT', val);
  },
  GPS_LNG: function GPS_LNG(val) {
    return map('GPS_LNG', val);
  },
  IP: function IP(val) {
    return map('IP', val);
  },
