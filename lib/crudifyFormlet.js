 ...
 */
const initAction = require('./crudifyAction');
module.exports = (thorin, opt) => {
  ...
  let FormletAction;
  process.nextTick(() => {
    FormletAction = initAction(thorin, opt);
  });
  ...
  actions.create = function Create(namespaceObj, opt) {
    ...
    let actionObj = new FormletAction(actionId),
    ...
    process.nextTick(() => {
      actionObj.input({
        created_by: dispatcher.validate('STRING').error('DATA.INVALID', 'Missing created by information')
      });
      actionObj.use((intentObj, next) => {
        ...
        if (createdBy && !payload.created_by) {
  ...
  actions.read = function (namespaceObj, opt) {
    ...
    process.nextTick(() => {
      ...
      actionObj.use((intentObj, next) => {
        ...
        try {
          actionObj._runFilters('read.before', intentObj, payload);
        } catch (e) {
          intentObj.error(thorin.error(e));
          return actionObj._runSend('read.error', intentObj, next);
        }
        if (payload.formlet instanceof Array && payload.formlet.length === 0) {
          payload.formlet = '_all';
        }
        namespaceObj.read(payload).then((res) => {
          let entry = (typeof res.result === 'object' && res.result ? res.result : res),
    ...
    process.nextTick(() => {
      ...
      actionObj.use((intentObj, next) => {
        let fullData = intentObj.rawInput,
          ...
          formletObj = formlets[payload.formlet];
        let createdBy = intentObj.data('createdBy');
        if (createdBy && !payload.updated_by) {
        ...
        try {
          actionObj._runFilters('update.before', intentObj, payload);
        } catch (e) {
          intentObj.error(thorin.error(e));
          return actionObj._runSend('update.error', intentObj, next);
        }

        namespaceObj.update(payload).then((res) => {
          let entry = (typeof res.result === 'object' && res.result ? res.result : res),
            formletObj = formlets[entry.formlet];
          if (formletObj) {
            filterEntry(formletObj, entry);
  ...
  actions.delete = function (namespaceObj, opt) {
    ...
    process.nextTick(() => {
      ...
      actionObj.use((intentObj, next) => {
        ...
        try {
          actionObj._runFilters('delete.before', intentObj, payload);
        } catch (e) {
          intentObj.error(thorin.error(e));
          return actionObj._runSend('delete.error', intentObj, next);
        }
        if (!payload.formlet) delete payload.formlet;
        namespaceObj.delete(payload).then((res) => {
          let entry = (typeof res.result === 'object' && res.result ? res.result : res),
            formletObj = formlets[entry.formlet];
          if (formletObj) {
            filterEntry(formletObj, entry);
  ...
  actions.find = function (namespaceObj, opt) {
    ...
    process.nextTick(() => {
      ...
      actionObj.use((intentObj, next) => {
        ...
        try {
          actionObj._runFilters('find.before', intentObj, payload);
        } catch (e) {
          intentObj.error(thorin.error(e));
          return actionObj._runSend('find.error', intentObj, next);
        }

        namespaceObj.find(payload).then((res) => {
